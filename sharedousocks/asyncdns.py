...
from sharedousocks import common, lru_cache, eventloop, shell
...
class DNSResolver(object):
    ...
    def _parse_resolv(self):
        ...
        if 'WINDIR' in os.environ:
            ...
            reg_key_dns_servers = "NameServer"
            with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, reg_path) as reg_key_interfaces:
                for i in range(winreg.QueryInfoKey(reg_key_interfaces)[0]):
                    interface_key_name = winreg.EnumKey(reg_key_interfaces, i)
                    with winreg.OpenKey(reg_key_interfaces, interface_key_name) as interface_key:
                        try:
                            dns_server, _ = winreg.QueryValueEx(interface_key, reg_key_dns_servers)
                            self._servers += dns_server.split(',')
                        except Exception:
                            pass
            temp = []
            for _server in self._servers:
                if _server and not _server in temp:
                    temp.append(_server)
            self._servers = temp
