...
LOCAL="$PYTHON sharedousocks/local.py"
SERVER="$PYTHON sharedousocks/server.py"

assert "$LOCAL --version 2>&1 | grep sharedousocks | awk -F\" \" '{print \$1}'" "sharedousocks"
assert "$SERVER --version 2>&1 | grep sharedousocks | awk -F\" \" '{print \$1}'" "sharedousocks"
