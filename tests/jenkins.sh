...
coverage combine && coverage report --include=sharedousocks/*
rm -rf htmlcov
rm -rf tmp
coverage html --include=sharedousocks/*

coverage report --include=sharedousocks/* | tail -n1 | rev | cut -d' ' -f 1 | rev > /tmp/sharedousocks-coverage
